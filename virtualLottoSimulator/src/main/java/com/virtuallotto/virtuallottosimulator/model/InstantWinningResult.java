...
public class InstantWinningResult {
    ...
    public InstantWinningResult(WinningAndBonusNumber winningAndBonusNumber, int[] gameWinningPrizeList, List<List<Integer>> lottoList) {
    ...
    public void calculateUserWinningPrizeResult() {
        for (List<Integer> lotto : lottoList) {
            int numberOfMatchingNumbers = findNumberOfCommonElements(lotto, winningAndBonusNumber.getWinningNumber());
            int hasBonusNumber = 0;
            if (numberOfMatchingNumbers == Rank.getNumberOfMatchesRequiredFromIndex(2)) {
                hasBonusNumber = hasBonusNumberInLotto(winningAndBonusNumber.getBonusNumber(), lotto);
            }
